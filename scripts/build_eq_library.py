...
if can:
    ...
    for c in can['canonical']:
        cq = coq_ids.get(c['id'], ('', '', ''))
        out.append(f"| {c.get('code','')} | {c['id']} | {c.get('root_object','')} | {c.get('domain','')} | {c.get('canonical_text','')[:110].replace('|','\\|')} | {c.get('tier','')} | {cq[0]} {cq[1]} | {len(c.get('occurrences',[]))} |")
    out.append('')
