(* Z/M.01.v1 -- mapped_not_wrapped -> wrapped -- restates delta_sum (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Calculus.v) -- parents: Z *)
(* name: delta_sum (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of delta_sum`
   ...
   is `exact delta_sum.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Calculus.

Definition Z__M_01_v1_restates_delta_sum_type := ltac:(let t := type of (@delta_sum) in exact t).
Theorem Z__M_01_v1_restates_delta_sum : Z__M_01_v1_restates_delta_sum_type.
Proof. exact (@delta_sum). Qed.

Print Assumptions Z__M_01_v1_restates_delta_sum.
