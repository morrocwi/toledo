(* L_R/M.19.v1 -- mapped_not_wrapped -> wrapped -- restates skew_diag_zero (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Harvest.v) -- parents: L_R *)
(* name: skew_diag_zero (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of skew_diag_zero`
   ...
   is `exact skew_diag_zero.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Harvest.

Definition L_R__M_19_v1_restates_skew_diag_zero_type := ltac:(let t := type of (@skew_diag_zero) in exact t).
Theorem L_R__M_19_v1_restates_skew_diag_zero : L_R__M_19_v1_restates_skew_diag_zero_type.
Proof. exact (@skew_diag_zero). Qed.

Print Assumptions L_R__M_19_v1_restates_skew_diag_zero.
