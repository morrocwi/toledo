(* L_R/M.13.v1 -- mapped_not_wrapped -> wrapped -- restates repeated_event_zero (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Harvest.v) -- parents: L_R *)
(* name: repeated_event_zero (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of repeated_event_zero`
   ...
   is `exact repeated_event_zero.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Harvest.

Definition L_R__M_13_v1_restates_repeated_event_zero_type := ltac:(let t := type of (@repeated_event_zero) in exact t).
Theorem L_R__M_13_v1_restates_repeated_event_zero : L_R__M_13_v1_restates_repeated_event_zero_type.
Proof. exact (@repeated_event_zero). Qed.

Print Assumptions L_R__M_13_v1_restates_repeated_event_zero.
