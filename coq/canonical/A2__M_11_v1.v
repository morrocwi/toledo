(* A2/M.11.v1 -- mapped_not_wrapped -> wrapped -- restates fold_add_split (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Reduction.v) -- parents: A2 *)
(* name: fold_add_split (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of fold_add_split`
   ...
   is `exact fold_add_split.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Reduction.

Definition A2__M_11_v1_restates_fold_add_split_type := ltac:(let t := type of (@fold_add_split) in exact t).
Theorem A2__M_11_v1_restates_fold_add_split : A2__M_11_v1_restates_fold_add_split_type.
Proof. exact (@fold_add_split). Qed.

Print Assumptions A2__M_11_v1_restates_fold_add_split.
