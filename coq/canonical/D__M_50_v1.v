(* D/M.50.v1 -- mapped_not_wrapped -> wrapped -- restates aut_inv_left (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_FiniteWitnesses3.v) -- parents: D *)
(* name: aut_inv_left (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of aut_inv_left`
   ...
   is `exact aut_inv_left.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_FiniteWitnesses3.

Definition D__M_50_v1_restates_aut_inv_left_type := ltac:(let t := type of (@aut_inv_left) in exact t).
Theorem D__M_50_v1_restates_aut_inv_left : D__M_50_v1_restates_aut_inv_left_type.
Proof. exact (@aut_inv_left). Qed.

Print Assumptions D__M_50_v1_restates_aut_inv_left.
