(* Z/M.04.v1 -- mapped_not_wrapped -> wrapped -- restates Deps_sum (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Calculus.v) -- parents: Z *)
(* name: Deps_sum (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of Deps_sum`
   ...
   is `exact Deps_sum.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Calculus.

Definition Z__M_04_v1_restates_Deps_sum_type := ltac:(let t := type of (@Deps_sum) in exact t).
Theorem Z__M_04_v1_restates_Deps_sum : Z__M_04_v1_restates_Deps_sum_type.
Proof. exact (@Deps_sum). Qed.

Print Assumptions Z__M_04_v1_restates_Deps_sum.
