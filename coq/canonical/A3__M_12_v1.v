(* A3/M.12.v1 -- mapped_not_wrapped -> wrapped -- restates tautology_sound (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Reduction.v) -- parents: A3 *)
(* name: tautology_sound (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of tautology_sound`
   ...
   is `exact tautology_sound.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Reduction.

Definition A3__M_12_v1_restates_tautology_sound_type := ltac:(let t := type of (@tautology_sound) in exact t).
Theorem A3__M_12_v1_restates_tautology_sound : A3__M_12_v1_restates_tautology_sound_type.
Proof. exact (@tautology_sound). Qed.

Print Assumptions A3__M_12_v1_restates_tautology_sound.
