(* D/M.47.v1 -- mapped_not_wrapped -> wrapped -- restates aut_assoc (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_FiniteWitnesses3.v) -- parents: D *)
(* name: aut_assoc (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of aut_assoc`
   ...
   is `exact aut_assoc.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_FiniteWitnesses3.

Definition D__M_47_v1_restates_aut_assoc_type := ltac:(let t := type of (@aut_assoc) in exact t).
Theorem D__M_47_v1_restates_aut_assoc : D__M_47_v1_restates_aut_assoc_type.
Proof. exact (@aut_assoc). Qed.

Print Assumptions D__M_47_v1_restates_aut_assoc.
