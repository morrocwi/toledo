(* A2/M.19.v1 -- mapped_not_wrapped -> wrapped -- restates horner_scaled (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Reduction.v) -- parents: A2 *)
(* name: horner_scaled (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of horner_scaled`
   ...
   is `exact horner_scaled.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Reduction.

Definition A2__M_19_v1_restates_horner_scaled_type := ltac:(let t := type of (@horner_scaled) in exact t).
Lemma A2__M_19_v1_restates_horner_scaled : A2__M_19_v1_restates_horner_scaled_type.
Proof. exact (@horner_scaled). Qed.

Print Assumptions A2__M_19_v1_restates_horner_scaled.
