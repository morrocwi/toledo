...
for f in $FILES; do
  ...
  out=$(coqc -q -Q . MRC -Q ../master-river MR -R ../solver-arc RDL -Q ../readout_universe/evidence URR -Q ../readout_genesis/formal ReadoutGenesis.Formal -R ../information-discrete-math IDM "$f" 2>&1)
  status=$?
  if [ $status -eq 0 ]; then
    OK=$((OK+1))
    echo "OK   $f" | tee -a "$LOG"
