(* D/M.07.v1 -- mapped_not_wrapped -> wrapped -- restates k_bound (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_ApproxCount.v) -- parents: D *)
(* name: k_bound (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of k_bound`
   ...
   is `exact k_bound.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_ApproxCount.

Definition D__M_07_v1_restates_k_bound_type := ltac:(let t := type of (@k_bound) in exact t).
Lemma D__M_07_v1_restates_k_bound : D__M_07_v1_restates_k_bound_type.
Proof. exact (@k_bound). Qed.

Print Assumptions D__M_07_v1_restates_k_bound.
