(* R/M.32.v1 -- mapped_not_wrapped -> wrapped -- restates refine_stable (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Certified.v) -- parents: R *)
(* name: refine_stable (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of refine_stable`
   ...
   is `exact refine_stable.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Certified.

Definition R__M_32_v1_restates_refine_stable_type := ltac:(let t := type of (@refine_stable) in exact t).
Theorem R__M_32_v1_restates_refine_stable : R__M_32_v1_restates_refine_stable_type.
Proof. exact (@refine_stable). Qed.

Print Assumptions R__M_32_v1_restates_refine_stable.
