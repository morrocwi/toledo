(* D/M.39.v1 -- mapped_not_wrapped -> wrapped -- restates finite_yoneda (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_FiniteWitnesses.v) -- parents: D *)
(* name: finite_yoneda (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of finite_yoneda`
   ...
   is `exact finite_yoneda.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_FiniteWitnesses.

Definition D__M_39_v1_restates_finite_yoneda_type := ltac:(let t := type of (@finite_yoneda) in exact t).
Theorem D__M_39_v1_restates_finite_yoneda : D__M_39_v1_restates_finite_yoneda_type.
Proof. exact (@finite_yoneda). Qed.

Print Assumptions D__M_39_v1_restates_finite_yoneda.
