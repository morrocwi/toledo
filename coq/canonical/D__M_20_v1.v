(* D/M.20.v1 -- mapped_not_wrapped -> wrapped -- restates bcube_complete (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_DeclarationBound.v) -- parents: D *)
(* name: bcube_complete (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of bcube_complete`
   ...
   is `exact bcube_complete.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_DeclarationBound.

Definition D__M_20_v1_restates_bcube_complete_type := ltac:(let t := type of (@bcube_complete) in exact t).
Lemma D__M_20_v1_restates_bcube_complete : D__M_20_v1_restates_bcube_complete_type.
Proof. exact (@bcube_complete). Qed.

Print Assumptions D__M_20_v1_restates_bcube_complete.
