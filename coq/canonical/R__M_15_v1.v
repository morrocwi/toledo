(* R/M.15.v1 -- mapped_not_wrapped -> wrapped -- restates one_le_Sk (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Certified.v) -- parents: R *)
(* name: one_le_Sk (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of one_le_Sk`
   ...
   is `exact one_le_Sk.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Certified.

Definition R__M_15_v1_restates_one_le_Sk_type := ltac:(let t := type of (@one_le_Sk) in exact t).
Lemma R__M_15_v1_restates_one_le_Sk : R__M_15_v1_restates_one_le_Sk_type.
Proof. exact (@one_le_Sk). Qed.

Print Assumptions R__M_15_v1_restates_one_le_Sk.
