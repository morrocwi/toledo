(* EQ-015/P.64.v1 — wrapped_related — wraps evidence/DRL_Forced_Master.v.discrete_Lagrange_d_Alembert_forced_master_theorem — parents: EQ-015 *)
(* name: Cubic-Hermite beam FEM element (4-DOF stiffness matrix, consistent UDL fixed-end forces; exact nodal values for point/uniform loads) *)
(* tier: untagged (textbook_system -- use_text: imported element matrices, assembled + solved numerically, `textbook_system`) *)
(* statement (Toledo canonical, latest): Cubic-Hermite beam FEM element (4-DOF stiffness matrix, consistent UDL fixed-end forces; exact nodal values for point/uniform loads) *)
...
Definition EQ_015__P_64_v1_reads := discrete_Lagrange_d_Alembert_forced_master_theorem.
