(* A2/M.05.v1 -- mapped_not_wrapped -> wrapped -- restates foldmin_le_elem (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Reduction.v) -- parents: A2 *)
(* name: foldmin_le_elem (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of foldmin_le_elem`
   ...
   is `exact foldmin_le_elem.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Reduction.

Definition A2__M_05_v1_restates_foldmin_le_elem_type := ltac:(let t := type of (@foldmin_le_elem) in exact t).
Lemma A2__M_05_v1_restates_foldmin_le_elem : A2__M_05_v1_restates_foldmin_le_elem_type.
Proof. exact (@foldmin_le_elem). Qed.

Print Assumptions A2__M_05_v1_restates_foldmin_le_elem.
