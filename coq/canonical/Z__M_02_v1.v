(* Z/M.02.v1 -- mapped_not_wrapped -> wrapped -- restates delta_scalar (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Calculus.v) -- parents: Z *)
(* name: delta_scalar (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of delta_scalar`
   ...
   is `exact delta_scalar.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Calculus.

Definition Z__M_02_v1_restates_delta_scalar_type := ltac:(let t := type of (@delta_scalar) in exact t).
Theorem Z__M_02_v1_restates_delta_scalar : Z__M_02_v1_restates_delta_scalar_type.
Proof. exact (@delta_scalar). Qed.

Print Assumptions Z__M_02_v1_restates_delta_scalar.
