(* EQ-015/P.72.v1 — wrapped_related — wraps evidence/DRL_Forced_Master.v.discrete_Lagrange_d_Alembert_forced_master_theorem — parents: EQ-015 *)
(* name: Simple-pendulum and mass-spring SHM period (`T=2π√(L/g)`, `T=2π√(m/k)`) and closed-form SHM motion (`x=A cos(ωt+φ)`) *)
(* tier: untagged (textbook_closure -- use_text: numeric closures, `textbook_closure`) *)
(* statement (Toledo canonical, latest): Simple-pendulum and mass-spring SHM period (T=2\pi\sqrt(L/g), T=2\pi\sqrt(m/k)) and closed-form SHM motion (x=A cos(\omegat+\varphi)) *)
...
Definition EQ_015__P_72_v1_reads := discrete_Lagrange_d_Alembert_forced_master_theorem.
