(* EQ-015/W.46.v1 — wrapped_related — wraps evidence/DRL_Forced_Master.v.discrete_Lagrange_d_Alembert_forced_master_theorem — parents: EQ-015/M.03.v1, EQ-015/W.10.v1 *)
(* name: Abundance-Without-Agency (AWA_t) tuple *)
(* tier: Definition (definition) *)
(* statement (Toledo canonical, latest): AWA_t = < Y(up), Gamma^eff adequate, O^H(down), X^H(down), D^H(up), Lambda^live_H(down), A^corr_H(down), r_H(down), S_H(down) > *)
...
Definition EQ_015__W_46_v1_reads := discrete_Lagrange_d_Alembert_forced_master_theorem.
