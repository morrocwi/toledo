(* D/M.19.v1 -- mapped_not_wrapped -> wrapped -- restates bcube_all_len (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_DeclarationBound.v) -- parents: D *)
(* name: bcube_all_len (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of bcube_all_len`
   ...
   is `exact bcube_all_len.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_DeclarationBound.

Definition D__M_19_v1_restates_bcube_all_len_type := ltac:(let t := type of (@bcube_all_len) in exact t).
Lemma D__M_19_v1_restates_bcube_all_len : D__M_19_v1_restates_bcube_all_len_type.
Proof. exact (@bcube_all_len). Qed.

Print Assumptions D__M_19_v1_restates_bcube_all_len.
