(* D/M.71.v1 -- mapped_not_wrapped -> wrapped -- restates classify_bot_iff (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_ResolvedCount.v) -- parents: D *)
(* name: classify_bot_iff (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of classify_bot_iff`
   ...
   is `exact classify_bot_iff.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_ResolvedCount.

Definition D__M_71_v1_restates_classify_bot_iff_type := ltac:(let t := type of (@classify_bot_iff) in exact t).
Theorem D__M_71_v1_restates_classify_bot_iff : D__M_71_v1_restates_classify_bot_iff_type.
Proof. exact (@classify_bot_iff). Qed.

Print Assumptions D__M_71_v1_restates_classify_bot_iff.
