(* A2/M.09.v1 -- mapped_not_wrapped -> wrapped -- restates fold_ext (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Reduction.v) -- parents: A2 *)
(* name: fold_ext (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of fold_ext`
   ...
   is `exact fold_ext.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Reduction.

Definition A2__M_09_v1_restates_fold_ext_type := ltac:(let t := type of (@fold_ext) in exact t).
Lemma A2__M_09_v1_restates_fold_ext : A2__M_09_v1_restates_fold_ext_type.
Proof. exact (@fold_ext). Qed.

Print Assumptions A2__M_09_v1_restates_fold_ext.
