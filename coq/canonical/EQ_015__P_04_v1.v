(* EQ-015/P.04.v1 — wrapped_related — wraps evidence/DRL_Forced_Master.v.discrete_Lagrange_d_Alembert_forced_master_theorem — parents: EQ-015 *)
(* name: General relativity field equations `G_μν+Λg_μν=8πGT_μν` *)
(* tier: untagged (<no tier tag in registry 'use' text> -- use_text: imported conclusion only (`[Dr]`/`[AX/import]`); never derived) *)
(* statement (Toledo canonical, latest): General relativity field equations G_\mu\nu+\Lambdag_\mu\nu=8\piGT_\mu\nu *)
...
Definition EQ_015__P_04_v1_reads := discrete_Lagrange_d_Alembert_forced_master_theorem.
