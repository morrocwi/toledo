(* D/M.43.v1 -- mapped_not_wrapped -> wrapped -- restates tape_count_succ (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_FiniteWitnesses2.v) -- parents: D *)
(* name: tape_count_succ (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of tape_count_succ`
   ...
   is `exact tape_count_succ.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_FiniteWitnesses2.

Definition D__M_43_v1_restates_tape_count_succ_type := ltac:(let t := type of (@tape_count_succ) in exact t).
Theorem D__M_43_v1_restates_tape_count_succ : D__M_43_v1_restates_tape_count_succ_type.
Proof. exact (@tape_count_succ). Qed.

Print Assumptions D__M_43_v1_restates_tape_count_succ.
