(* delta_R/M.04.v1 -- mapped_not_wrapped -> wrapped -- restates rdl_non_explosion (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Logic.v) -- parents: delta_R *)
(* name: rdl_non_explosion (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of rdl_non_explosion`
   ...
   is `exact rdl_non_explosion.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Logic.

Definition delta_R__M_04_v1_restates_rdl_non_explosion_type := ltac:(let t := type of (@rdl_non_explosion) in exact t).
Theorem delta_R__M_04_v1_restates_rdl_non_explosion : delta_R__M_04_v1_restates_rdl_non_explosion_type.
Proof. exact (@rdl_non_explosion). Qed.

Print Assumptions delta_R__M_04_v1_restates_rdl_non_explosion.
