(* R/M.19.v1 -- mapped_not_wrapped -> wrapped -- restates div_le_self (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Certified.v) -- parents: R *)
(* name: div_le_self (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of div_le_self`
   ...
   is `exact div_le_self.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Certified.

Definition R__M_19_v1_restates_div_le_self_type := ltac:(let t := type of (@div_le_self) in exact t).
Lemma R__M_19_v1_restates_div_le_self : R__M_19_v1_restates_div_le_self_type.
Proof. exact (@div_le_self). Qed.

Print Assumptions R__M_19_v1_restates_div_le_self.
