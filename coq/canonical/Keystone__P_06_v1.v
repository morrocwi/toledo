(* Keystone/P.06.v1 -- mapped_not_wrapped -> wrapped -- restates inner_pos (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Hilbert.v) -- parents: Keystone *)
(* name: inner_pos (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of inner_pos`
   ...
   is `exact inner_pos.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Hilbert.

Definition Keystone__P_06_v1_restates_inner_pos_type := ltac:(let t := type of (@inner_pos) in exact t).
Theorem Keystone__P_06_v1_restates_inner_pos : Keystone__P_06_v1_restates_inner_pos_type.
Proof. exact (@inner_pos). Qed.

Print Assumptions Keystone__P_06_v1_restates_inner_pos.
