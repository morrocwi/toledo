(* L_R/M.12.v1 -- mapped_not_wrapped -> wrapped -- restates orient_collinear_aff (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Geometry.v) -- parents: L_R *)
(* name: orient_collinear_aff (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of orient_collinear_aff`
   ...
   is `exact orient_collinear_aff.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Geometry.

Definition L_R__M_12_v1_restates_orient_collinear_aff_type := ltac:(let t := type of (@orient_collinear_aff) in exact t).
Lemma L_R__M_12_v1_restates_orient_collinear_aff : L_R__M_12_v1_restates_orient_collinear_aff_type.
Proof. exact (@orient_collinear_aff). Qed.

Print Assumptions L_R__M_12_v1_restates_orient_collinear_aff.
