(* EQ-015/P.67.v1 — wrapped_related — wraps evidence/DRL_Forced_Master.v.discrete_Lagrange_d_Alembert_forced_master_theorem — parents: EQ-015 *)
(* name: 1D Ising model + transfer-matrix exact solution (`f = -kT ln lambda_+`, `xi = -1/ln tanh(beta J)` at `h=0`) *)
(* tier: untagged (textbook_system -- use_text: exact closed forms evaluated numerically, `textbook_system`) *)
(* statement (Toledo canonical, latest): 1D Ising model + transfer-matrix exact solution (f = -kT ln lambda_+, xi = -1/ln tanh(beta J) at h=0) *)
...
Definition EQ_015__P_67_v1_reads := discrete_Lagrange_d_Alembert_forced_master_theorem.
