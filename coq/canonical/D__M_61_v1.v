(* D/M.61.v1 -- mapped_not_wrapped -> wrapped -- restates neg_moves_iff (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_ReadoutMinimality.v) -- parents: D *)
(* name: neg_moves_iff (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of neg_moves_iff`
   ...
   is `exact neg_moves_iff.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_ReadoutMinimality.

Definition D__M_61_v1_restates_neg_moves_iff_type := ltac:(let t := type of (@neg_moves_iff) in exact t).
Theorem D__M_61_v1_restates_neg_moves_iff : D__M_61_v1_restates_neg_moves_iff_type.
Proof. exact (@neg_moves_iff). Qed.

Print Assumptions D__M_61_v1_restates_neg_moves_iff.
