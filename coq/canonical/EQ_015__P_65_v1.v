(* EQ-015/P.65.v1 — wrapped_related — wraps evidence/DRL_Forced_Master.v.discrete_Lagrange_d_Alembert_forced_master_theorem — parents: EQ-015 *)
(* name: 1D axial-bar FEM element (`k_e = EA/L` two-node element) *)
(* tier: untagged (textbook_system -- use_text: imported element form, assembled + solved numerically, `textbook_system`) *)
(* statement (Toledo canonical, latest): 1D axial-bar FEM element (k_e = EA/L two-node element) *)
...
Definition EQ_015__P_65_v1_reads := discrete_Lagrange_d_Alembert_forced_master_theorem.
