(* R/M.35.v1 -- mapped_not_wrapped -> wrapped -- restates radd_comm (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Continuum.v) -- parents: R *)
(* name: radd_comm (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of radd_comm`
   ...
   is `exact radd_comm.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Continuum.

Definition R__M_35_v1_restates_radd_comm_type := ltac:(let t := type of (@radd_comm) in exact t).
Theorem R__M_35_v1_restates_radd_comm : R__M_35_v1_restates_radd_comm_type.
Proof. exact (@radd_comm). Qed.

Print Assumptions R__M_35_v1_restates_radd_comm.
