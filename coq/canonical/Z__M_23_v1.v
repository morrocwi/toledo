(* Z/M.23.v1 -- mapped_not_wrapped -> wrapped -- restates bottleneck_distrib (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Tropical.v) -- parents: Z *)
(* name: bottleneck_distrib (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of bottleneck_distrib`
   ...
   is `exact bottleneck_distrib.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Tropical.

Definition Z__M_23_v1_restates_bottleneck_distrib_type := ltac:(let t := type of (@bottleneck_distrib) in exact t).
Theorem Z__M_23_v1_restates_bottleneck_distrib : Z__M_23_v1_restates_bottleneck_distrib_type.
Proof. exact (@bottleneck_distrib). Qed.

Print Assumptions Z__M_23_v1_restates_bottleneck_distrib.
