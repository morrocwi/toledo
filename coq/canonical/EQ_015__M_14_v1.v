(* EQ-015/M.14.v1 — wrapped_related — wraps evidence/DRL_Forced_Master.v.discrete_Lagrange_d_Alembert_forced_master_theorem — parents: EQ-015/M.13.v1 *)
(* name: Discrete Retention Hamiltonian (conserved companion of the DRL Lagrangian) *)
(* tier: finite_diagnostic (finite_diagnostic — measured relative drift 1.5e-4) *)
(* statement (Toledo canonical, latest): H = M\cdotv\Phi^{T}v\Psi + K\cdot\Phi^{T}L_R\Psi + k_{2}\cdot\Phi^{T}\Psi (conserved quantity) *)
...
Definition EQ_015__M_14_v1_reads := discrete_Lagrange_d_Alembert_forced_master_theorem.
