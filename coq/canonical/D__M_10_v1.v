(* D/M.10.v1 -- mapped_not_wrapped -> wrapped -- restates fam_in (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_ApproxCount.v) -- parents: D *)
(* name: fam_in (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of fam_in`
   ...
   is `exact fam_in.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_ApproxCount.

Definition D__M_10_v1_restates_fam_in_type := ltac:(let t := type of (@fam_in) in exact t).
Lemma D__M_10_v1_restates_fam_in : D__M_10_v1_restates_fam_in_type.
Proof. exact (@fam_in). Qed.

Print Assumptions D__M_10_v1_restates_fam_in.
