(* A2/M.20.v1 -- mapped_not_wrapped -> wrapped -- restates horner_is_poly (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Reduction.v) -- parents: A2 *)
(* name: horner_is_poly (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of horner_is_poly`
   ...
   is `exact horner_is_poly.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Reduction.

Definition A2__M_20_v1_restates_horner_is_poly_type := ltac:(let t := type of (@horner_is_poly) in exact t).
Theorem A2__M_20_v1_restates_horner_is_poly : A2__M_20_v1_restates_horner_is_poly_type.
Proof. exact (@horner_is_poly). Qed.

Print Assumptions A2__M_20_v1_restates_horner_is_poly.
