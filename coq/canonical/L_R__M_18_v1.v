(* L_R/M.18.v1 -- mapped_not_wrapped -> wrapped -- restates skew_antisym (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Harvest.v) -- parents: L_R *)
(* name: skew_antisym (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of skew_antisym`
   ...
   is `exact skew_antisym.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Harvest.

Definition L_R__M_18_v1_restates_skew_antisym_type := ltac:(let t := type of (@skew_antisym) in exact t).
Theorem L_R__M_18_v1_restates_skew_antisym : L_R__M_18_v1_restates_skew_antisym_type.
Proof. exact (@skew_antisym). Qed.

Print Assumptions L_R__M_18_v1_restates_skew_antisym.
