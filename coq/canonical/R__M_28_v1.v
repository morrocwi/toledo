(* R/M.28.v1 -- mapped_not_wrapped -> wrapped -- restates iter_sq_valbound (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Certified.v) -- parents: R *)
(* name: iter_sq_valbound (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of iter_sq_valbound`
   ...
   is `exact iter_sq_valbound.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Certified.

Definition R__M_28_v1_restates_iter_sq_valbound_type := ltac:(let t := type of (@iter_sq_valbound) in exact t).
Lemma R__M_28_v1_restates_iter_sq_valbound : R__M_28_v1_restates_iter_sq_valbound_type.
Proof. exact (@iter_sq_valbound). Qed.

Print Assumptions R__M_28_v1_restates_iter_sq_valbound.
