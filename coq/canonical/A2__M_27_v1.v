(* A2/M.27.v1 -- mapped_not_wrapped -> wrapped -- restates fold_split_even_odd (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Reduction.v) -- parents: A2 *)
(* name: fold_split_even_odd (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of fold_split_even_odd`
   ...
   is `exact fold_split_even_odd.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Reduction.

Definition A2__M_27_v1_restates_fold_split_even_odd_type := ltac:(let t := type of (@fold_split_even_odd) in exact t).
Theorem A2__M_27_v1_restates_fold_split_even_odd : A2__M_27_v1_restates_fold_split_even_odd_type.
Proof. exact (@fold_split_even_odd). Qed.

Print Assumptions A2__M_27_v1_restates_fold_split_even_odd.
