(* D/M.04.v1 -- mapped_not_wrapped -> wrapped -- restates weight_repeat_true (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_ApproxCount.v) -- parents: D *)
(* name: weight_repeat_true (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of weight_repeat_true`
   ...
   is `exact weight_repeat_true.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_ApproxCount.

Definition D__M_04_v1_restates_weight_repeat_true_type := ltac:(let t := type of (@weight_repeat_true) in exact t).
Lemma D__M_04_v1_restates_weight_repeat_true : D__M_04_v1_restates_weight_repeat_true_type.
Proof. exact (@weight_repeat_true). Qed.

Print Assumptions D__M_04_v1_restates_weight_repeat_true.
