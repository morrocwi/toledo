(* R/M.34.v1 -- mapped_not_wrapped -> wrapped -- restates rmul_at (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Continuum.v) -- parents: R *)
(* name: rmul_at (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of rmul_at`
   ...
   is `exact rmul_at.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Continuum.

Definition R__M_34_v1_restates_rmul_at_type := ltac:(let t := type of (@rmul_at) in exact t).
Theorem R__M_34_v1_restates_rmul_at : R__M_34_v1_restates_rmul_at_type.
Proof. exact (@rmul_at). Qed.

Print Assumptions R__M_34_v1_restates_rmul_at.
