(* D/M.24.v1 -- mapped_not_wrapped -> wrapped -- restates bcube_nodup (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_DeclarationBound.v) -- parents: D *)
(* name: bcube_nodup (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of bcube_nodup`
   ...
   is `exact bcube_nodup.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_DeclarationBound.

Definition D__M_24_v1_restates_bcube_nodup_type := ltac:(let t := type of (@bcube_nodup) in exact t).
Lemma D__M_24_v1_restates_bcube_nodup : D__M_24_v1_restates_bcube_nodup_type.
Proof. exact (@bcube_nodup). Qed.

Print Assumptions D__M_24_v1_restates_bcube_nodup.
