(* D/M.17.v1 -- mapped_not_wrapped -> wrapped -- restates bcube_length (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_DeclarationBound.v) -- parents: D *)
(* name: bcube_length (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of bcube_length`
   ...
   is `exact bcube_length.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_DeclarationBound.

Definition D__M_17_v1_restates_bcube_length_type := ltac:(let t := type of (@bcube_length) in exact t).
Lemma D__M_17_v1_restates_bcube_length : D__M_17_v1_restates_bcube_length_type.
Proof. exact (@bcube_length). Qed.

Print Assumptions D__M_17_v1_restates_bcube_length.
