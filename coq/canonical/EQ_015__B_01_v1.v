(* EQ-015/B.01.v1 — wrapped_related — wraps evidence/DRL_Forced_Master.v.discrete_Lagrange_d_Alembert_forced_master_theorem — parents: EQ-015 *)
(* name: Health-stream theorem: setpoint_is_fixed *)
(* tier: untagged (untagged -- theorem name only, no source tier tag captured in src_solver_arc.json health_stream inventory) *)
(* statement (Toledo canonical, latest): setpoint_is_fixed (Coq theorem name only; no LaTeX statement extracted in this pass -- statement text lives in the .v file body, out of BBL-195/198 source-inventory scope, and reading it would require opening solver arc (private)'s Coq source beyond the inventory JSON this build was scoped to) *)
...
Definition EQ_015__B_01_v1_reads := discrete_Lagrange_d_Alembert_forced_master_theorem.
