(* delta_R/M.09.v1 -- mapped_not_wrapped -> wrapped -- restates graph_faithful (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_SetsFunctions.v) -- parents: delta_R *)
(* name: graph_faithful (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of graph_faithful`
   ...
   is `exact graph_faithful.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_SetsFunctions.

Definition delta_R__M_09_v1_restates_graph_faithful_type := ltac:(let t := type of (@graph_faithful) in exact t).
Theorem delta_R__M_09_v1_restates_graph_faithful : delta_R__M_09_v1_restates_graph_faithful_type.
Proof. exact (@graph_faithful). Qed.

Print Assumptions delta_R__M_09_v1_restates_graph_faithful.
