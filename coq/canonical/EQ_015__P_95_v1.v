(* EQ-015/P.95.v1 — wrapped_related — wraps evidence/DRL_Forced_Master.v.discrete_Lagrange_d_Alembert_forced_master_theorem — parents: EQ-015 *)
(* name: Periodogram / power spectral density estimation (one-sided PSD scaling used here follows the periodogram lineage; Hann window per J. von Hann, popularized by Blackman & Tukey) *)
(* tier: untagged (textbook_system -- use_text: one-sided PSD computed numerically from the FFT with standard periodogram scaling; optional periodic-Hann taper, `textbook_system`) *)
(* statement (Toledo canonical, latest): Periodogram / power spectral density estimation (one-sided PSD scaling used here follows the periodogram lineage; Hann window per J. von Hann, popularized by Blackman & Tukey) *)
...
Definition EQ_015__P_95_v1_reads := discrete_Lagrange_d_Alembert_forced_master_theorem.
