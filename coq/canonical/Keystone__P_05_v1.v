(* Keystone/P.05.v1 -- mapped_not_wrapped -> wrapped -- restates inner_linear_l (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Hilbert.v) -- parents: Keystone *)
(* name: inner_linear_l (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of inner_linear_l`
   ...
   is `exact inner_linear_l.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Hilbert.

Definition Keystone__P_05_v1_restates_inner_linear_l_type := ltac:(let t := type of (@inner_linear_l) in exact t).
Theorem Keystone__P_05_v1_restates_inner_linear_l : Keystone__P_05_v1_restates_inner_linear_l_type.
Proof. exact (@inner_linear_l). Qed.

Print Assumptions Keystone__P_05_v1_restates_inner_linear_l.
