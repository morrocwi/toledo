(* Q/M.16.v1 -- mapped_not_wrapped -> wrapped -- restates inject_nat_nonzero (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Matrix.v) -- parents: Q *)
(* name: inject_nat_nonzero (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of inject_nat_nonzero`
   ...
   is `exact inject_nat_nonzero.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Matrix.

Definition Q__M_16_v1_restates_inject_nat_nonzero_type := ltac:(let t := type of (@inject_nat_nonzero) in exact t).
Lemma Q__M_16_v1_restates_inject_nat_nonzero : Q__M_16_v1_restates_inject_nat_nonzero_type.
Proof. exact (@inject_nat_nonzero). Qed.

Print Assumptions Q__M_16_v1_restates_inject_nat_nonzero.
