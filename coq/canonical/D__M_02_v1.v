(* D/M.02.v1 -- mapped_not_wrapped -> wrapped -- restates weight_cons (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_ApproxCount.v) -- parents: D *)
(* name: weight_cons (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of weight_cons`
   ...
   is `exact weight_cons.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_ApproxCount.

Definition D__M_02_v1_restates_weight_cons_type := ltac:(let t := type of (@weight_cons) in exact t).
Lemma D__M_02_v1_restates_weight_cons : D__M_02_v1_restates_weight_cons_type.
Proof. exact (@weight_cons). Qed.

Print Assumptions D__M_02_v1_restates_weight_cons.
