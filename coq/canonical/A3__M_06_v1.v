(* A3/M.06.v1 -- mapped_not_wrapped -> wrapped -- restates decide_reflect (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Reduction.v) -- parents: A3 *)
(* name: decide_reflect (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of decide_reflect`
   ...
   is `exact decide_reflect.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Reduction.

Definition A3__M_06_v1_restates_decide_reflect_type := ltac:(let t := type of (@decide_reflect) in exact t).
Theorem A3__M_06_v1_restates_decide_reflect : A3__M_06_v1_restates_decide_reflect_type.
Proof. exact (@decide_reflect). Qed.

Print Assumptions A3__M_06_v1_restates_decide_reflect.
