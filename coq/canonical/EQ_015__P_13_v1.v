(* EQ-015/P.13.v1 — wrapped_related — wraps evidence/DRL_Forced_Master.v.discrete_Lagrange_d_Alembert_forced_master_theorem — parents: EQ-015 *)
(* name: Nielsen–Ninomiya fermion-doubling no-go (locality + translation invariance + Hermiticity + exact naïve chiral anticommutation ⟹ `2^d` species) *)
(* tier: untagged (<no tier tag in registry 'use' text> -- use_text: cited as the OBSTRUCTION the GW construction evades (we do NOT keep exact naïve `{D,Γ}=0`); the `2^d` corner count and its `16→1+15` lifting at d=4 are mechanized as arithmetic, not a re-derivation of the no-go) *)
(* statement (Toledo canonical, latest): Nielsen–Ninomiya fermion-doubling no-go (locality + translation invariance + Hermiticity + exact naïve chiral anticommutation ⟹ 2^d species) *)
...
Definition EQ_015__P_13_v1_reads := discrete_Lagrange_d_Alembert_forced_master_theorem.
