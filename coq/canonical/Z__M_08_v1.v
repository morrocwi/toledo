(* Z/M.08.v1 -- mapped_not_wrapped -> wrapped -- restates PSum_delta_telescope (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Calculus.v) -- parents: Z *)
(* name: PSum_delta_telescope (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of PSum_delta_telescope`
   ...
   is `exact PSum_delta_telescope.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Calculus.

Definition Z__M_08_v1_restates_PSum_delta_telescope_type := ltac:(let t := type of (@PSum_delta_telescope) in exact t).
Lemma Z__M_08_v1_restates_PSum_delta_telescope : Z__M_08_v1_restates_PSum_delta_telescope_type.
Proof. exact (@PSum_delta_telescope). Qed.

Print Assumptions Z__M_08_v1_restates_PSum_delta_telescope.
