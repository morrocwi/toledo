(* EQ-015/P.66.v1 — wrapped_related — wraps evidence/DRL_Forced_Master.v.discrete_Lagrange_d_Alembert_forced_master_theorem — parents: EQ-015 *)
(* name: Modal analysis closed forms for uniform spring-mass chains, `omega_n^2 = (2k/m)(1-cos(n pi/(N+1)))`, and the generalized eigenproblem `K phi = omega^2 M phi` *)
(* tier: untagged (textbook_system -- use_text: eigenproblem solved numerically via `scipy.linalg.eigh(K,M)`, closed form used only as an independent test oracle, `textbook_system`) *)
(* statement (Toledo canonical, latest): Modal analysis closed forms for uniform spring-mass chains, omega_n^2 = (2k/m)(1-cos(n pi/(N+1))), and the generalized eigenproblem K phi = omega^2 M phi *)
...
Definition EQ_015__P_66_v1_reads := discrete_Lagrange_d_Alembert_forced_master_theorem.
