(* R/M.22.v1 -- mapped_not_wrapped -> wrapped -- restates exp_tail_certified (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Certified.v) -- parents: R *)
(* name: exp_tail_certified (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of exp_tail_certified`
   ...
   is `exact exp_tail_certified.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Certified.

Definition R__M_22_v1_restates_exp_tail_certified_type := ltac:(let t := type of (@exp_tail_certified) in exact t).
Theorem R__M_22_v1_restates_exp_tail_certified : R__M_22_v1_restates_exp_tail_certified_type.
Proof. exact (@exp_tail_certified). Qed.

Print Assumptions R__M_22_v1_restates_exp_tail_certified.
