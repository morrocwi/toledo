(* D/M.78.v1 -- mapped_not_wrapped -> wrapped -- restates count_if_split (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_ResolvedCount.v) -- parents: D *)
(* name: count_if_split (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of count_if_split`
   ...
   is `exact count_if_split.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_ResolvedCount.

Definition D__M_78_v1_restates_count_if_split_type := ltac:(let t := type of (@count_if_split) in exact t).
Lemma D__M_78_v1_restates_count_if_split : D__M_78_v1_restates_count_if_split_type.
Proof. exact (@count_if_split). Qed.

Print Assumptions D__M_78_v1_restates_count_if_split.
