(* Q/M.14.v1 -- mapped_not_wrapped -> wrapped -- restates Sum_const (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Matrix.v) -- parents: Q *)
(* name: Sum_const (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of Sum_const`
   ...
   is `exact Sum_const.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Matrix.

Definition Q__M_14_v1_restates_Sum_const_type := ltac:(let t := type of (@Sum_const) in exact t).
Lemma Q__M_14_v1_restates_Sum_const : Q__M_14_v1_restates_Sum_const_type.
Proof. exact (@Sum_const). Qed.

Print Assumptions Q__M_14_v1_restates_Sum_const.
