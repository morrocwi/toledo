(* A2/M.06.v1 -- mapped_not_wrapped -> wrapped -- restates sum_is_fold (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Reduction.v) -- parents: A2 *)
(* name: sum_is_fold (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of sum_is_fold`
   ...
   is `exact sum_is_fold.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Reduction.

Definition A2__M_06_v1_restates_sum_is_fold_type := ltac:(let t := type of (@sum_is_fold) in exact t).
Theorem A2__M_06_v1_restates_sum_is_fold : A2__M_06_v1_restates_sum_is_fold_type.
Proof. exact (@sum_is_fold). Qed.

Print Assumptions A2__M_06_v1_restates_sum_is_fold.
