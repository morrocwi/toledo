(* Keystone/M.02.v1 -- mapped_not_wrapped -> wrapped -- restates keystone_edge (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Keystone.v) -- parents: Keystone *)
(* name: keystone_edge (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of keystone_edge`
   ...
   is `exact keystone_edge.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Keystone.

Definition Keystone__M_02_v1_restates_keystone_edge_type := ltac:(let t := type of (@keystone_edge) in exact t).
Lemma Keystone__M_02_v1_restates_keystone_edge : Keystone__M_02_v1_restates_keystone_edge_type.
Proof. exact (@keystone_edge). Qed.

Print Assumptions Keystone__M_02_v1_restates_keystone_edge.
