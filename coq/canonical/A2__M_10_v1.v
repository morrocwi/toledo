(* A2/M.10.v1 -- mapped_not_wrapped -> wrapped -- restates fold_linear (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Reduction.v) -- parents: A2 *)
(* name: fold_linear (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of fold_linear`
   ...
   is `exact fold_linear.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Reduction.

Definition A2__M_10_v1_restates_fold_linear_type := ltac:(let t := type of (@fold_linear) in exact t).
Theorem A2__M_10_v1_restates_fold_linear : A2__M_10_v1_restates_fold_linear_type.
Proof. exact (@fold_linear). Qed.

Print Assumptions A2__M_10_v1_restates_fold_linear.
