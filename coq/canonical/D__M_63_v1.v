(* D/M.63.v1 -- mapped_not_wrapped -> wrapped -- restates minimal_three_values (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_ReadoutMinimality.v) -- parents: D *)
(* name: minimal_three_values (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of minimal_three_values`
   ...
   is `exact minimal_three_values.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_ReadoutMinimality.

Definition D__M_63_v1_restates_minimal_three_values_type := ltac:(let t := type of (@minimal_three_values) in exact t).
Theorem D__M_63_v1_restates_minimal_three_values : D__M_63_v1_restates_minimal_three_values_type.
Proof. exact (@minimal_three_values). Qed.

Print Assumptions D__M_63_v1_restates_minimal_three_values.
