(* EQ-015/P.19.v1 — wrapped_related — wraps evidence/DRL_Forced_Master.v.discrete_Lagrange_d_Alembert_forced_master_theorem — parents: EQ-015 *)
(* name: Perron–Frobenius / doubly-stochastic consensus (a primitive doubly-stochastic `P` has a simple eigenvalue 1 with `P|_{1^⊥}` spectral radius `<1` ⇒ convergence to the mean) *)
(* tier: untagged (<no tier tag in registry 'use' text> -- use_text: standard averaging/consensus theorem used to drive per-sector limiting speeds to a common `c_*`; cited for position, the sector-graph reading is this project's own — equality of speeds derived, the value `c_*` NOT predicted) *)
(* statement (Toledo canonical, latest): Perron–Frobenius / doubly-stochastic consensus (a primitive doubly-stochastic P has a simple eigenvalue 1 with P|_{1^⊥} spectral radius <1 ⇒ convergence to the mean) *)
...
Definition EQ_015__P_19_v1_reads := discrete_Lagrange_d_Alembert_forced_master_theorem.
