(* Keystone/M.06.v1 -- mapped_not_wrapped -> wrapped -- restates I_edge_nonneg (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Keystone.v) -- parents: Keystone *)
(* name: I_edge_nonneg (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of I_edge_nonneg`
   ...
   is `exact I_edge_nonneg.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Keystone.

Definition Keystone__M_06_v1_restates_I_edge_nonneg_type := ltac:(let t := type of (@I_edge_nonneg) in exact t).
Lemma Keystone__M_06_v1_restates_I_edge_nonneg : Keystone__M_06_v1_restates_I_edge_nonneg_type.
Proof. exact (@I_edge_nonneg). Qed.

Print Assumptions Keystone__M_06_v1_restates_I_edge_nonneg.
