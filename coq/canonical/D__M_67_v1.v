(* D/M.67.v1 -- mapped_not_wrapped -> wrapped -- restates bottom_is_least (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_ReadoutMinimality.v) -- parents: D *)
(* name: bottom_is_least (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of bottom_is_least`
   ...
   is `exact bottom_is_least.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_ReadoutMinimality.

Definition D__M_67_v1_restates_bottom_is_least_type := ltac:(let t := type of (@bottom_is_least) in exact t).
Theorem D__M_67_v1_restates_bottom_is_least : D__M_67_v1_restates_bottom_is_least_type.
Proof. exact (@bottom_is_least). Qed.

Print Assumptions D__M_67_v1_restates_bottom_is_least.
