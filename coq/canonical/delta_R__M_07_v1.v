(* delta_R/M.07.v1 -- mapped_not_wrapped -> wrapped -- restates graph_total (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_SetsFunctions.v) -- parents: delta_R *)
(* name: graph_total (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of graph_total`
   ...
   is `exact graph_total.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_SetsFunctions.

Definition delta_R__M_07_v1_restates_graph_total_type := ltac:(let t := type of (@graph_total) in exact t).
Theorem delta_R__M_07_v1_restates_graph_total : delta_R__M_07_v1_restates_graph_total_type.
Proof. exact (@graph_total). Qed.

Print Assumptions delta_R__M_07_v1_restates_graph_total.
