(* EQ-015/P.108.v1 — wrapped_related — wraps evidence/DRL_Forced_Master.v.discrete_Lagrange_d_Alembert_forced_master_theorem — parents: EQ-015 *)
(* name: Relativistic Doppler shift (radial) `f_obs = f_source sqrt((1-beta)/(1+beta))` *)
(* tier: untagged (textbook_closure -- use_text: numeric closure, `textbook_closure`) *)
(* statement (Toledo canonical, latest): Relativistic Doppler shift (radial) f_obs = f_source sqrt((1-beta)/(1+beta)) *)
...
Definition EQ_015__P_108_v1_reads := discrete_Lagrange_d_Alembert_forced_master_theorem.
