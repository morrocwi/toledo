(* A2/M.02.v1 -- mapped_not_wrapped -> wrapped -- restates fold_right_perm (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Reduction.v) -- parents: A2 *)
(* name: fold_right_perm (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of fold_right_perm`
   ...
   is `exact fold_right_perm.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Reduction.

Definition A2__M_02_v1_restates_fold_right_perm_type := ltac:(let t := type of (@fold_right_perm) in exact t).
Theorem A2__M_02_v1_restates_fold_right_perm : A2__M_02_v1_restates_fold_right_perm_type.
Proof. exact (@fold_right_perm). Qed.

Print Assumptions A2__M_02_v1_restates_fold_right_perm.
