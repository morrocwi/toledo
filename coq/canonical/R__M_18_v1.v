(* R/M.18.v1 -- mapped_not_wrapped -> wrapped -- restates pos_Sk (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Certified.v) -- parents: R *)
(* name: pos_Sk (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of pos_Sk`
   ...
   is `exact pos_Sk.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Certified.

Definition R__M_18_v1_restates_pos_Sk_type := ltac:(let t := type of (@pos_Sk) in exact t).
Lemma R__M_18_v1_restates_pos_Sk : R__M_18_v1_restates_pos_Sk_type.
Proof. exact (@pos_Sk). Qed.

Print Assumptions R__M_18_v1_restates_pos_Sk.
