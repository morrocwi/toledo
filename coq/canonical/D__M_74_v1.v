(* D/M.74.v1 -- mapped_not_wrapped -> wrapped -- restates classify_plus_sound (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_ResolvedCount.v) -- parents: D *)
(* name: classify_plus_sound (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of classify_plus_sound`
   ...
   is `exact classify_plus_sound.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_ResolvedCount.

Definition D__M_74_v1_restates_classify_plus_sound_type := ltac:(let t := type of (@classify_plus_sound) in exact t).
Theorem D__M_74_v1_restates_classify_plus_sound : D__M_74_v1_restates_classify_plus_sound_type.
Proof. exact (@classify_plus_sound). Qed.

Print Assumptions D__M_74_v1_restates_classify_plus_sound.
