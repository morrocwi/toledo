(* L_R/M.20.v1 -- mapped_not_wrapped -> wrapped -- restates laplacian_symmetric (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Matrix.v) -- parents: L_R *)
(* name: laplacian_symmetric (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of laplacian_symmetric`
   ...
   is `exact laplacian_symmetric.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Matrix.

Definition L_R__M_20_v1_restates_laplacian_symmetric_type := ltac:(let t := type of (@laplacian_symmetric) in exact t).
Theorem L_R__M_20_v1_restates_laplacian_symmetric : L_R__M_20_v1_restates_laplacian_symmetric_type.
Proof. exact (@laplacian_symmetric). Qed.

Print Assumptions L_R__M_20_v1_restates_laplacian_symmetric.
