(* EQ-015/P.111.v1 — wrapped_related — wraps evidence/DRL_Forced_Master.v.discrete_Lagrange_d_Alembert_forced_master_theorem — parents: EQ-015 *)
(* name: Schwarzschild radius `r_s = 2GM/c^2` *)
(* tier: untagged (textbook_closure -- use_text: numeric closure, `textbook_closure` (note: `InfoAnalysisLift.v` already imports the Schwarzschild metric itself as a `Definition`; this is an independent, undecorated numeric evaluation of the horizon-radius formula, same source equation)) *)
(* statement (Toledo canonical, latest): Schwarzschild radius r_s = 2GM/c^2 *)
...
Definition EQ_015__P_111_v1_reads := discrete_Lagrange_d_Alembert_forced_master_theorem.
