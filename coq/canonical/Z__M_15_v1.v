(* Z/M.15.v1 -- mapped_not_wrapped -> wrapped -- restates tmax_assoc (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Tropical.v) -- parents: Z *)
(* name: tmax_assoc (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of tmax_assoc`
   ...
   is `exact tmax_assoc.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Tropical.

Definition Z__M_15_v1_restates_tmax_assoc_type := ltac:(let t := type of (@tmax_assoc) in exact t).
Theorem Z__M_15_v1_restates_tmax_assoc : Z__M_15_v1_restates_tmax_assoc_type.
Proof. exact (@tmax_assoc). Qed.

Print Assumptions Z__M_15_v1_restates_tmax_assoc.
