(* EQ-015/P.113.v1 — wrapped_related — wraps evidence/DRL_Forced_Master.v.discrete_Lagrange_d_Alembert_forced_master_theorem — parents: EQ-015 *)
(* name: Debye model heat capacity, low-temperature `T^3` limit `C_V = (12 pi^4/5) N k_B (T/theta_D)^3` *)
(* tier: untagged (textbook_closure -- use_text: numeric closure, `textbook_closure`; test uses silver's tabulated Debye temperature theta_D=225 K (C. Kittel, *Introduction to Solid State Physics*, Table 1)) *)
(* statement (Toledo canonical, latest): Debye model heat capacity, low-temperature T^3 limit C_V = (12 pi^4/5) N k_B (T/theta_D)^3 *)
...
Definition EQ_015__P_113_v1_reads := discrete_Lagrange_d_Alembert_forced_master_theorem.
