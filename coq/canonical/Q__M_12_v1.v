(* Q/M.12.v1 -- mapped_not_wrapped -> wrapped -- restates mid_left (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Matrix.v) -- parents: Q *)
(* name: mid_left (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of mid_left`
   ...
   is `exact mid_left.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Matrix.

Definition Q__M_12_v1_restates_mid_left_type := ltac:(let t := type of (@mid_left) in exact t).
Theorem Q__M_12_v1_restates_mid_left : Q__M_12_v1_restates_mid_left_type.
Proof. exact (@mid_left). Qed.

Print Assumptions Q__M_12_v1_restates_mid_left.
