(* Q/M.18.v1 -- mapped_not_wrapped -> wrapped -- restates twirl_idempotent (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Matrix.v) -- parents: Q *)
(* name: twirl_idempotent (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of twirl_idempotent`
   ...
   is `exact twirl_idempotent.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Matrix.

Definition Q__M_18_v1_restates_twirl_idempotent_type := ltac:(let t := type of (@twirl_idempotent) in exact t).
Theorem Q__M_18_v1_restates_twirl_idempotent : Q__M_18_v1_restates_twirl_idempotent_type.
Proof. exact (@twirl_idempotent). Qed.

Print Assumptions Q__M_18_v1_restates_twirl_idempotent.
