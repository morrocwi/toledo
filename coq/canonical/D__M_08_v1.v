(* D/M.08.v1 -- mapped_not_wrapped -> wrapped -- restates fam_elem_length (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_ApproxCount.v) -- parents: D *)
(* name: fam_elem_length (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of fam_elem_length`
   ...
   is `exact fam_elem_length.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_ApproxCount.

Definition D__M_08_v1_restates_fam_elem_length_type := ltac:(let t := type of (@fam_elem_length) in exact t).
Lemma D__M_08_v1_restates_fam_elem_length : D__M_08_v1_restates_fam_elem_length_type.
Proof. exact (@fam_elem_length). Qed.

Print Assumptions D__M_08_v1_restates_fam_elem_length.
