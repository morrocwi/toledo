(* A2/M.18.v1 -- mapped_not_wrapped -> wrapped -- restates dot_scale (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Reduction.v) -- parents: A2 *)
(* name: dot_scale (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of dot_scale`
   ...
   is `exact dot_scale.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Reduction.

Definition A2__M_18_v1_restates_dot_scale_type := ltac:(let t := type of (@dot_scale) in exact t).
Theorem A2__M_18_v1_restates_dot_scale : A2__M_18_v1_restates_dot_scale_type.
Proof. exact (@dot_scale). Qed.

Print Assumptions A2__M_18_v1_restates_dot_scale.
