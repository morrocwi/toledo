(* R/M.31.v1 -- mapped_not_wrapped -> wrapped -- restates abs_tailsum_le (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Certified.v) -- parents: R *)
(* name: abs_tailsum_le (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of abs_tailsum_le`
   ...
   is `exact abs_tailsum_le.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Certified.

Definition R__M_31_v1_restates_abs_tailsum_le_type := ltac:(let t := type of (@abs_tailsum_le) in exact t).
Lemma R__M_31_v1_restates_abs_tailsum_le : R__M_31_v1_restates_abs_tailsum_le_type.
Proof. exact (@abs_tailsum_le). Qed.

Print Assumptions R__M_31_v1_restates_abs_tailsum_le.
