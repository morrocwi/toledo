(* D/M.55.v1 -- mapped_not_wrapped -> wrapped -- restates discrete_floor (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Genesis.v) -- parents: D *)
(* name: discrete_floor (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of discrete_floor`
   ...
   is `exact discrete_floor.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Genesis.

Definition D__M_55_v1_restates_discrete_floor_type := ltac:(let t := type of (@discrete_floor) in exact t).
Theorem D__M_55_v1_restates_discrete_floor : D__M_55_v1_restates_discrete_floor_type.
Proof. exact (@discrete_floor). Qed.

Print Assumptions D__M_55_v1_restates_discrete_floor.
