(* Q/M.04.v1 -- mapped_not_wrapped -> wrapped -- restates Sum_zero (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Matrix.v) -- parents: Q *)
(* name: Sum_zero (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of Sum_zero`
   ...
   is `exact Sum_zero.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Matrix.

Definition Q__M_04_v1_restates_Sum_zero_type := ltac:(let t := type of (@Sum_zero) in exact t).
Lemma Q__M_04_v1_restates_Sum_zero : Q__M_04_v1_restates_Sum_zero_type.
Proof. exact (@Sum_zero). Qed.

Print Assumptions Q__M_04_v1_restates_Sum_zero.
