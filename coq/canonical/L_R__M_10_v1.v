(* L_R/M.10.v1 -- mapped_not_wrapped -> wrapped -- restates orient_scale (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Geometry.v) -- parents: L_R *)
(* name: orient_scale (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of orient_scale`
   ...
   is `exact orient_scale.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Geometry.

Definition L_R__M_10_v1_restates_orient_scale_type := ltac:(let t := type of (@orient_scale) in exact t).
Lemma L_R__M_10_v1_restates_orient_scale : L_R__M_10_v1_restates_orient_scale_type.
Proof. exact (@orient_scale). Qed.

Print Assumptions L_R__M_10_v1_restates_orient_scale.
