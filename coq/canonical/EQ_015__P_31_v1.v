(* EQ-015/P.31.v1 — wrapped_related — wraps evidence/DRL_Forced_Master.v.discrete_Lagrange_d_Alembert_forced_master_theorem — parents: EQ-015 *)
(* name: Elitzur's theorem (a local gauge symmetry cannot break spontaneously; only gauge-invariant order parameters like `⟨H†H⟩` are physical) *)
(* tier: untagged (<no tier tag in registry 'use' text> -- use_text: cited to state the vacuum result gauge-invariantly (`⟨H†H⟩>0`, not `⟨H⟩≠0`); the closure-pressure selection `Π_cl>α ⇒ r*>0` and its convexity/positivity are this framework's own, mechanized over ℚ) *)
(* statement (Toledo canonical, latest): Elitzur's theorem (a local gauge symmetry cannot break spontaneously; only gauge-invariant order parameters like \langleH^{\dagger}H\rangle are physical) *)
...
Definition EQ_015__P_31_v1_reads := discrete_Lagrange_d_Alembert_forced_master_theorem.
