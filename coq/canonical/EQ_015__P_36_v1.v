(* EQ-015/P.36.v1 — wrapped_related — wraps evidence/DRL_Forced_Master.v.discrete_Lagrange_d_Alembert_forced_master_theorem — parents: EQ-015 *)
(* name: GUT hypercharge normalization `T₁(R) = (3/5)Y²` (SU(5) embedding factor needed to compare U(1)_Y on the same Dynkin-index footing as SU(2)/SU(3)) *)
(* tier: untagged (<no tier tag in registry 'use' text> -- use_text: imported external normalization convention, plugged in directly, not derived) *)
(* statement (Toledo canonical, latest): GUT hypercharge normalization T_{1}(R) = (3/5)Y^{2} (SU(5) embedding factor needed to compare U(1)_Y on the same Dynkin-index footing as SU(2)/SU(3)) *)
...
Definition EQ_015__P_36_v1_reads := discrete_Lagrange_d_Alembert_forced_master_theorem.
