(* Z/M.18.v1 -- mapped_not_wrapped -> wrapped -- restates tadd_assoc (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Tropical.v) -- parents: Z *)
(* name: tadd_assoc (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of tadd_assoc`
   ...
   is `exact tadd_assoc.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Tropical.

Definition Z__M_18_v1_restates_tadd_assoc_type := ltac:(let t := type of (@tadd_assoc) in exact t).
Theorem Z__M_18_v1_restates_tadd_assoc : Z__M_18_v1_restates_tadd_assoc_type.
Proof. exact (@tadd_assoc). Qed.

Print Assumptions Z__M_18_v1_restates_tadd_assoc.
