(* Z/M.19.v1 -- mapped_not_wrapped -> wrapped -- restates tadd_comm (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Tropical.v) -- parents: Z *)
(* name: tadd_comm (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of tadd_comm`
   ...
   is `exact tadd_comm.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Tropical.

Definition Z__M_19_v1_restates_tadd_comm_type := ltac:(let t := type of (@tadd_comm) in exact t).
Theorem Z__M_19_v1_restates_tadd_comm : Z__M_19_v1_restates_tadd_comm_type.
Proof. exact (@tadd_comm). Qed.

Print Assumptions Z__M_19_v1_restates_tadd_comm.
