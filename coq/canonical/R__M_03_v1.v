(* R/M.03.v1 -- mapped_not_wrapped -> wrapped -- restates apriori_stable (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Apriori.v) -- parents: R *)
(* name: apriori_stable (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of apriori_stable`
   ...
   is `exact apriori_stable.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Apriori.

Definition R__M_03_v1_restates_apriori_stable_type := ltac:(let t := type of (@apriori_stable) in exact t).
Theorem R__M_03_v1_restates_apriori_stable : R__M_03_v1_restates_apriori_stable_type.
Proof. exact (@apriori_stable). Qed.

Print Assumptions R__M_03_v1_restates_apriori_stable.
