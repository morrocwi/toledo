(* R/M.10.v1 -- mapped_not_wrapped -> wrapped -- restates FTCC_eps_exact (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Bridge.v) -- parents: R *)
(* name: FTCC_eps_exact (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of FTCC_eps_exact`
   ...
   is `exact FTCC_eps_exact.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Bridge.

Definition R__M_10_v1_restates_FTCC_eps_exact_type := ltac:(let t := type of (@FTCC_eps_exact) in exact t).
Theorem R__M_10_v1_restates_FTCC_eps_exact : R__M_10_v1_restates_FTCC_eps_exact_type.
Proof. exact (@FTCC_eps_exact). Qed.

Print Assumptions R__M_10_v1_restates_FTCC_eps_exact.
