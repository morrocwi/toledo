(* A2/M.01.v1 -- mapped_not_wrapped -> wrapped -- restates op_swap (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Reduction.v) -- parents: A2 *)
(* name: op_swap (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of op_swap`
   ...
   is `exact op_swap.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Reduction.

Definition A2__M_01_v1_restates_op_swap_type := ltac:(let t := type of (@op_swap) in exact t).
Lemma A2__M_01_v1_restates_op_swap : A2__M_01_v1_restates_op_swap_type.
Proof. exact (@op_swap). Qed.

Print Assumptions A2__M_01_v1_restates_op_swap.
