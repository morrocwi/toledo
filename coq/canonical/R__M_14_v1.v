(* R/M.14.v1 -- mapped_not_wrapped -> wrapped -- restates geom_majorant_tail (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Certified.v) -- parents: R *)
(* name: geom_majorant_tail (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of geom_majorant_tail`
   ...
   is `exact geom_majorant_tail.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Certified.

Definition R__M_14_v1_restates_geom_majorant_tail_type := ltac:(let t := type of (@geom_majorant_tail) in exact t).
Theorem R__M_14_v1_restates_geom_majorant_tail : R__M_14_v1_restates_geom_majorant_tail_type.
Proof. exact (@geom_majorant_tail). Qed.

Print Assumptions R__M_14_v1_restates_geom_majorant_tail.
