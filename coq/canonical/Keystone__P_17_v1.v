(* Keystone/P.17.v1 -- mapped_not_wrapped -> wrapped -- restates Qadd_nonneg (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_HilbertReadout.v) -- parents: Keystone *)
(* name: Qadd_nonneg (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of Qadd_nonneg`
   ...
   is `exact Qadd_nonneg.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_HilbertReadout.

Definition Keystone__P_17_v1_restates_Qadd_nonneg_type := ltac:(let t := type of (@Qadd_nonneg) in exact t).
Lemma Keystone__P_17_v1_restates_Qadd_nonneg : Keystone__P_17_v1_restates_Qadd_nonneg_type.
Proof. exact (@Qadd_nonneg). Qed.

Print Assumptions Keystone__P_17_v1_restates_Qadd_nonneg.
