(* Keystone/M.07.v1 -- mapped_not_wrapped -> wrapped -- restates keystone_nonneg (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Keystone.v) -- parents: Keystone *)
(* name: keystone_nonneg (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of keystone_nonneg`
   ...
   is `exact keystone_nonneg.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Keystone.

Definition Keystone__M_07_v1_restates_keystone_nonneg_type := ltac:(let t := type of (@keystone_nonneg) in exact t).
Theorem Keystone__M_07_v1_restates_keystone_nonneg : Keystone__M_07_v1_restates_keystone_nonneg_type.
Proof. exact (@keystone_nonneg). Qed.

Print Assumptions Keystone__M_07_v1_restates_keystone_nonneg.
