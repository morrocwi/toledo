(* R/M.04.v1 -- mapped_not_wrapped -> wrapped -- restates half_nonneg (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Apriori.v) -- parents: R *)
(* name: half_nonneg (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of half_nonneg`
   ...
   is `exact half_nonneg.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Apriori.

Definition R__M_04_v1_restates_half_nonneg_type := ltac:(let t := type of (@half_nonneg) in exact t).
Lemma R__M_04_v1_restates_half_nonneg : R__M_04_v1_restates_half_nonneg_type.
Proof. exact (@half_nonneg). Qed.

Print Assumptions R__M_04_v1_restates_half_nonneg.
