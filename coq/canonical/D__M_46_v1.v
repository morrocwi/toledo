(* D/M.46.v1 -- mapped_not_wrapped -> wrapped -- restates measure_additive (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_FiniteWitnesses3.v) -- parents: D *)
(* name: measure_additive (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of measure_additive`
   ...
   is `exact measure_additive.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_FiniteWitnesses3.

Definition D__M_46_v1_restates_measure_additive_type := ltac:(let t := type of (@measure_additive) in exact t).
Theorem D__M_46_v1_restates_measure_additive : D__M_46_v1_restates_measure_additive_type.
Proof. exact (@measure_additive). Qed.

Print Assumptions D__M_46_v1_restates_measure_additive.
