(* D/M.44.v1 -- mapped_not_wrapped -> wrapped -- restates tape_no_terminal (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_FiniteWitnesses2.v) -- parents: D *)
(* name: tape_no_terminal (Corollary) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of tape_no_terminal`
   ...
   is `exact tape_no_terminal.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_FiniteWitnesses2.

Definition D__M_44_v1_restates_tape_no_terminal_type := ltac:(let t := type of (@tape_no_terminal) in exact t).
Corollary D__M_44_v1_restates_tape_no_terminal : D__M_44_v1_restates_tape_no_terminal_type.
Proof. exact (@tape_no_terminal). Qed.

Print Assumptions D__M_44_v1_restates_tape_no_terminal.
