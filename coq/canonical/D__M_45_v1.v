(* D/M.45.v1 -- mapped_not_wrapped -> wrapped -- restates same_set_same_size (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_FiniteWitnesses2.v) -- parents: D *)
(* name: same_set_same_size (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of same_set_same_size`
   ...
   is `exact same_set_same_size.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_FiniteWitnesses2.

Definition D__M_45_v1_restates_same_set_same_size_type := ltac:(let t := type of (@same_set_same_size) in exact t).
Theorem D__M_45_v1_restates_same_set_same_size : D__M_45_v1_restates_same_set_same_size_type.
Proof. exact (@same_set_same_size). Qed.

Print Assumptions D__M_45_v1_restates_same_set_same_size.
