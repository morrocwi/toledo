(* R/M.33.v1 -- mapped_not_wrapped -> wrapped -- restates radd_at (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Continuum.v) -- parents: R *)
(* name: radd_at (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of radd_at`
   ...
   is `exact radd_at.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Continuum.

Definition R__M_33_v1_restates_radd_at_type := ltac:(let t := type of (@radd_at) in exact t).
Theorem R__M_33_v1_restates_radd_at : R__M_33_v1_restates_radd_at_type.
Proof. exact (@radd_at). Qed.

Print Assumptions R__M_33_v1_restates_radd_at.
