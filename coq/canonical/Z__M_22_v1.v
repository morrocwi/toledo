(* Z/M.22.v1 -- mapped_not_wrapped -> wrapped -- restates maxplus_distrib (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Tropical.v) -- parents: Z *)
(* name: maxplus_distrib (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of maxplus_distrib`
   ...
   is `exact maxplus_distrib.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Tropical.

Definition Z__M_22_v1_restates_maxplus_distrib_type := ltac:(let t := type of (@maxplus_distrib) in exact t).
Theorem Z__M_22_v1_restates_maxplus_distrib : Z__M_22_v1_restates_maxplus_distrib_type.
Proof. exact (@maxplus_distrib). Qed.

Print Assumptions Z__M_22_v1_restates_maxplus_distrib.
