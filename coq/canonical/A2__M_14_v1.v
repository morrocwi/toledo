(* A2/M.14.v1 -- mapped_not_wrapped -> wrapped -- restates foldmax_in (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Reduction.v) -- parents: A2 *)
(* name: foldmax_in (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of foldmax_in`
   ...
   is `exact foldmax_in.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Reduction.

Definition A2__M_14_v1_restates_foldmax_in_type := ltac:(let t := type of (@foldmax_in) in exact t).
Lemma A2__M_14_v1_restates_foldmax_in : A2__M_14_v1_restates_foldmax_in_type.
Proof. exact (@foldmax_in). Qed.

Print Assumptions A2__M_14_v1_restates_foldmax_in.
