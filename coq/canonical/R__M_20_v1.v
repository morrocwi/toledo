(* R/M.20.v1 -- mapped_not_wrapped -> wrapped -- restates exp_term_nonneg (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Certified.v) -- parents: R *)
(* name: exp_term_nonneg (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of exp_term_nonneg`
   ...
   is `exact exp_term_nonneg.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Certified.

Definition R__M_20_v1_restates_exp_term_nonneg_type := ltac:(let t := type of (@exp_term_nonneg) in exact t).
Lemma R__M_20_v1_restates_exp_term_nonneg : R__M_20_v1_restates_exp_term_nonneg_type.
Proof. exact (@exp_term_nonneg). Qed.

Print Assumptions R__M_20_v1_restates_exp_term_nonneg.
