(* EQ-015/P.99.v1 — wrapped_related — wraps evidence/DRL_Forced_Master.v.discrete_Lagrange_d_Alembert_forced_master_theorem — parents: EQ-015 *)
(* name: Hydrogen fine-structure energy shift `dE_fs = -(Ry alpha^2/n^4)(n/(j+1/2) - 3/4)` (relativistic kinetic + spin-orbit, combined first-order result) *)
(* tier: untagged (textbook_closure -- use_text: numeric closure, `textbook_closure`) *)
(* statement (Toledo canonical, latest): Hydrogen fine-structure energy shift dE_fs = -(Ry alpha^2/n^4)(n/(j+1/2) - 3/4) (relativistic kinetic + spin-orbit, combined first-order result) *)
...
Definition EQ_015__P_99_v1_reads := discrete_Lagrange_d_Alembert_forced_master_theorem.
