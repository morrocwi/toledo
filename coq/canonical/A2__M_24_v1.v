(* A2/M.24.v1 -- mapped_not_wrapped -> wrapped -- restates relax_nonincreasing (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Reduction.v) -- parents: A2 *)
(* name: relax_nonincreasing (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of relax_nonincreasing`
   ...
   is `exact relax_nonincreasing.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Reduction.

Definition A2__M_24_v1_restates_relax_nonincreasing_type := ltac:(let t := type of (@relax_nonincreasing) in exact t).
Lemma A2__M_24_v1_restates_relax_nonincreasing : A2__M_24_v1_restates_relax_nonincreasing_type.
Proof. exact (@relax_nonincreasing). Qed.

Print Assumptions A2__M_24_v1_restates_relax_nonincreasing.
