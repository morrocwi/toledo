(* D/M.32.v1 -- mapped_not_wrapped -> wrapped -- restates flat_map_cons_length (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_DeclarationBound.v) -- parents: D *)
(* name: flat_map_cons_length (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of flat_map_cons_length`
   ...
   is `exact flat_map_cons_length.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_DeclarationBound.

Definition D__M_32_v1_restates_flat_map_cons_length_type := ltac:(let t := type of (@flat_map_cons_length) in exact t).
Lemma D__M_32_v1_restates_flat_map_cons_length : D__M_32_v1_restates_flat_map_cons_length_type.
Proof. exact (@flat_map_cons_length). Qed.

Print Assumptions D__M_32_v1_restates_flat_map_cons_length.
