(* EQ-015/H.37.v1 — wrapped_related — wraps evidence/DRL_Forced_Master.v.discrete_Lagrange_d_Alembert_forced_master_theorem — parents: EQ-015/M.03.v1 *)
(* name: Well-being state vector W_n and multiplicative maladaptive-cost index C_mal[n] *)
(* tier: Definition (definition) *)
(* statement (Toledo canonical, latest): W_n=(F_n,A_n^agency,M_n^meaning,R_n^relation,C_n^competence,Q_n^repair); C_mal[n]=rig(a_n)·gen(a_n)·mis(a_n)·loss(a_n) *)
...
Definition EQ_015__H_37_v1_reads := discrete_Lagrange_d_Alembert_forced_master_theorem.
