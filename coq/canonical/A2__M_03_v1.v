(* A2/M.03.v1 -- mapped_not_wrapped -> wrapped -- restates ftcc_Z (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Reduction.v) -- parents: A2 *)
(* name: ftcc_Z (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of ftcc_Z`
   ...
   is `exact ftcc_Z.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Reduction.

Definition A2__M_03_v1_restates_ftcc_Z_type := ltac:(let t := type of (@ftcc_Z) in exact t).
Theorem A2__M_03_v1_restates_ftcc_Z : A2__M_03_v1_restates_ftcc_Z_type.
Proof. exact (@ftcc_Z). Qed.

Print Assumptions A2__M_03_v1_restates_ftcc_Z.
