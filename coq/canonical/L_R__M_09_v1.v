(* L_R/M.09.v1 -- mapped_not_wrapped -> wrapped -- restates orient_translation (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Geometry.v) -- parents: L_R *)
(* name: orient_translation (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of orient_translation`
   ...
   is `exact orient_translation.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Geometry.

Definition L_R__M_09_v1_restates_orient_translation_type := ltac:(let t := type of (@orient_translation) in exact t).
Lemma L_R__M_09_v1_restates_orient_translation : L_R__M_09_v1_restates_orient_translation_type.
Proof. exact (@orient_translation). Qed.

Print Assumptions L_R__M_09_v1_restates_orient_translation.
