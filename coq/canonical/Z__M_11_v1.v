(* Z/M.11.v1 -- mapped_not_wrapped -> wrapped -- restates ring_distrib_Z (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_FiniteWitnesses3.v) -- parents: Z *)
(* name: ring_distrib_Z (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of ring_distrib_Z`
   ...
   is `exact ring_distrib_Z.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_FiniteWitnesses3.

Definition Z__M_11_v1_restates_ring_distrib_Z_type := ltac:(let t := type of (@ring_distrib_Z) in exact t).
Theorem Z__M_11_v1_restates_ring_distrib_Z : Z__M_11_v1_restates_ring_distrib_Z_type.
Proof. exact (@ring_distrib_Z). Qed.

Print Assumptions Z__M_11_v1_restates_ring_distrib_Z.
