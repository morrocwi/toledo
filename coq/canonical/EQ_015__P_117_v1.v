(* EQ-015/P.117.v1 — wrapped_related — wraps evidence/DRL_Forced_Master.v.discrete_Lagrange_d_Alembert_forced_master_theorem — parents: EQ-015 *)
(* name: Blasius laminar flat-plate boundary layer similarity solution (`delta_99≈5.0x/sqrt(Re_x)`, `Cf_x=0.664/sqrt(Re_x)`) *)
(* tier: untagged (textbook_closure -- use_text: numeric closure using the standard tabulated similarity-solution constants (e.g. F. M. White, *Viscous Fluid Flow* ), `textbook_closure`) *)
(* statement (Toledo canonical, latest): Blasius laminar flat-plate boundary layer similarity solution (delta_99\approx5.0x/sqrt(Re_x), Cf_x=0.664/sqrt(Re_x)) *)
...
Definition EQ_015__P_117_v1_reads := discrete_Lagrange_d_Alembert_forced_master_theorem.
