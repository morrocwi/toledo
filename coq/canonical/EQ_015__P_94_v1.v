(* EQ-015/P.94.v1 — wrapped_related — wraps evidence/DRL_Forced_Master.v.discrete_Lagrange_d_Alembert_forced_master_theorem — parents: EQ-015 *)
(* name: Fast Fourier Transform (Cooley–Tukey algorithm; used here via `numpy.fft.rfft`) *)
(* tier: untagged (textbook_system -- use_text: user-supplied signal's DFT computed numerically (`numpy.fft.rfft`), one-sided amplitude/PSD derived from it, `textbook_system`) *)
(* statement (Toledo canonical, latest): Fast Fourier Transform (Cooley–Tukey algorithm; used here via numpy.fft.rfft) *)
...
Definition EQ_015__P_94_v1_reads := discrete_Lagrange_d_Alembert_forced_master_theorem.
