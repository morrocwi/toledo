(* D/M.41.v1 -- mapped_not_wrapped -> wrapped -- restates semiring_distrib (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_FiniteWitnesses.v) -- parents: D *)
(* name: semiring_distrib (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of semiring_distrib`
   ...
   is `exact semiring_distrib.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_FiniteWitnesses.

Definition D__M_41_v1_restates_semiring_distrib_type := ltac:(let t := type of (@semiring_distrib) in exact t).
Theorem D__M_41_v1_restates_semiring_distrib : D__M_41_v1_restates_semiring_distrib_type.
Proof. exact (@semiring_distrib). Qed.

Print Assumptions D__M_41_v1_restates_semiring_distrib.
