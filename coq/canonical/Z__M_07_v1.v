(* Z/M.07.v1 -- mapped_not_wrapped -> wrapped -- restates PSum_ext (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Calculus.v) -- parents: Z *)
(* name: PSum_ext (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of PSum_ext`
   ...
   is `exact PSum_ext.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Calculus.

Definition Z__M_07_v1_restates_PSum_ext_type := ltac:(let t := type of (@PSum_ext) in exact t).
Lemma Z__M_07_v1_restates_PSum_ext : Z__M_07_v1_restates_PSum_ext_type.
Proof. exact (@PSum_ext). Qed.

Print Assumptions Z__M_07_v1_restates_PSum_ext.
