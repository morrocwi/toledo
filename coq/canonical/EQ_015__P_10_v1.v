(* EQ-015/P.10.v1 — wrapped_related — wraps evidence/DRL_Forced_Master.v.discrete_Lagrange_d_Alembert_forced_master_theorem — parents: EQ-015 *)
(* name: Peter–Weyl theorem / character convolution (class-function convolution multiplies representation coefficients) *)
(* tier: untagged (<no tier tag in registry 'use' text> -- use_text: used as the EXACT serial-blocking law `a_R^{(m)}=a_R^m` (⇒ `ρ_t(b)=ρ_t^{b²}`); standard harmonic analysis on compact groups, read here as retained-triality contraction under block-merging, not a coupling β-flow) *)
(* statement (Toledo canonical, latest): Peter–Weyl theorem / character convolution (class-function convolution multiplies representation coefficients) *)
...
Definition EQ_015__P_10_v1_reads := discrete_Lagrange_d_Alembert_forced_master_theorem.
