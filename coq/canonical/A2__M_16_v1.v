(* A2/M.16.v1 -- mapped_not_wrapped -> wrapped -- restates prod_list_perm (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Reduction.v) -- parents: A2 *)
(* name: prod_list_perm (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of prod_list_perm`
   ...
   is `exact prod_list_perm.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Reduction.

Definition A2__M_16_v1_restates_prod_list_perm_type := ltac:(let t := type of (@prod_list_perm) in exact t).
Theorem A2__M_16_v1_restates_prod_list_perm : A2__M_16_v1_restates_prod_list_perm_type.
Proof. exact (@prod_list_perm). Qed.

Print Assumptions A2__M_16_v1_restates_prod_list_perm.
