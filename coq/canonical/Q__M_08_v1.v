(* Q/M.08.v1 -- mapped_not_wrapped -> wrapped -- restates madd_comm (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Matrix.v) -- parents: Q *)
(* name: madd_comm (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of madd_comm`
   ...
   is `exact madd_comm.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Matrix.

Definition Q__M_08_v1_restates_madd_comm_type := ltac:(let t := type of (@madd_comm) in exact t).
Theorem Q__M_08_v1_restates_madd_comm : Q__M_08_v1_restates_madd_comm_type.
Proof. exact (@madd_comm). Qed.

Print Assumptions Q__M_08_v1_restates_madd_comm.
