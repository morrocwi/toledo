(* L_R/M.03.v1 -- mapped_not_wrapped -> wrapped -- restates orient_swap_bc (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Geometry.v) -- parents: L_R *)
(* name: orient_swap_bc (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of orient_swap_bc`
   ...
   is `exact orient_swap_bc.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Geometry.

Definition L_R__M_03_v1_restates_orient_swap_bc_type := ltac:(let t := type of (@orient_swap_bc) in exact t).
Lemma L_R__M_03_v1_restates_orient_swap_bc : L_R__M_03_v1_restates_orient_swap_bc_type.
Proof. exact (@orient_swap_bc). Qed.

Print Assumptions L_R__M_03_v1_restates_orient_swap_bc.
