(* D/M.40.v1 -- mapped_not_wrapped -> wrapped -- restates pigeonhole (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_FiniteWitnesses.v) -- parents: D *)
(* name: pigeonhole (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of pigeonhole`
   ...
   is `exact pigeonhole.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_FiniteWitnesses.

Definition D__M_40_v1_restates_pigeonhole_type := ltac:(let t := type of (@pigeonhole) in exact t).
Theorem D__M_40_v1_restates_pigeonhole : D__M_40_v1_restates_pigeonhole_type.
Proof. exact (@pigeonhole). Qed.

Print Assumptions D__M_40_v1_restates_pigeonhole.
