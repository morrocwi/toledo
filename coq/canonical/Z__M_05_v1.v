(* Z/M.05.v1 -- mapped_not_wrapped -> wrapped -- restates Deps_product (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Calculus.v) -- parents: Z *)
(* name: Deps_product (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of Deps_product`
   ...
   is `exact Deps_product.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Calculus.

Definition Z__M_05_v1_restates_Deps_product_type := ltac:(let t := type of (@Deps_product) in exact t).
Theorem Z__M_05_v1_restates_Deps_product : Z__M_05_v1_restates_Deps_product_type.
Proof. exact (@Deps_product). Qed.

Print Assumptions Z__M_05_v1_restates_Deps_product.
