(* A3/M.14.v1 -- mapped_not_wrapped -> wrapped -- restates witness_crt_sound (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Reduction.v) -- parents: A3 *)
(* name: witness_crt_sound (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of witness_crt_sound`
   ...
   is `exact witness_crt_sound.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Reduction.

Definition A3__M_14_v1_restates_witness_crt_sound_type := ltac:(let t := type of (@witness_crt_sound) in exact t).
Theorem A3__M_14_v1_restates_witness_crt_sound : A3__M_14_v1_restates_witness_crt_sound_type.
Proof. exact (@witness_crt_sound). Qed.

Print Assumptions A3__M_14_v1_restates_witness_crt_sound.
