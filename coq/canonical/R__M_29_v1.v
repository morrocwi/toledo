(* R/M.29.v1 -- mapped_not_wrapped -> wrapped -- restates iter_sq_certified (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Certified.v) -- parents: R *)
(* name: iter_sq_certified (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of iter_sq_certified`
   ...
   is `exact iter_sq_certified.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Certified.

Definition R__M_29_v1_restates_iter_sq_certified_type := ltac:(let t := type of (@iter_sq_certified) in exact t).
Theorem R__M_29_v1_restates_iter_sq_certified : R__M_29_v1_restates_iter_sq_certified_type.
Proof. exact (@iter_sq_certified). Qed.

Print Assumptions R__M_29_v1_restates_iter_sq_certified.
