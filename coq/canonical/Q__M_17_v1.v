(* Q/M.17.v1 -- mapped_not_wrapped -> wrapped -- restates trace_twirl (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Matrix.v) -- parents: Q *)
(* name: trace_twirl (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of trace_twirl`
   ...
   is `exact trace_twirl.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Matrix.

Definition Q__M_17_v1_restates_trace_twirl_type := ltac:(let t := type of (@trace_twirl) in exact t).
Lemma Q__M_17_v1_restates_trace_twirl : Q__M_17_v1_restates_trace_twirl_type.
Proof. exact (@trace_twirl). Qed.

Print Assumptions Q__M_17_v1_restates_trace_twirl.
