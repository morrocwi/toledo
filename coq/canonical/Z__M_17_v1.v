(* Z/M.17.v1 -- mapped_not_wrapped -> wrapped -- restates tmax_idem (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_Tropical.v) -- parents: Z *)
(* name: tmax_idem (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of tmax_idem`
   ...
   is `exact tmax_idem.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_Tropical.

Definition Z__M_17_v1_restates_tmax_idem_type := ltac:(let t := type of (@tmax_idem) in exact t).
Theorem Z__M_17_v1_restates_tmax_idem : Z__M_17_v1_restates_tmax_idem_type.
Proof. exact (@tmax_idem). Qed.

Print Assumptions Z__M_17_v1_restates_tmax_idem.
