(* D/M.03.v1 -- mapped_not_wrapped -> wrapped -- restates weight_app (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_ApproxCount.v) -- parents: D *)
(* name: weight_app (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of weight_app`
   ...
   is `exact weight_app.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_ApproxCount.

Definition D__M_03_v1_restates_weight_app_type := ltac:(let t := type of (@weight_app) in exact t).
Lemma D__M_03_v1_restates_weight_app : D__M_03_v1_restates_weight_app_type.
Proof. exact (@weight_app). Qed.

Print Assumptions D__M_03_v1_restates_weight_app.
