(* D/M.27.v1 -- mapped_not_wrapped -> wrapped -- restates NoDup_map_inj_on (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_DeclarationBound.v) -- parents: D *)
(* name: NoDup_map_inj_on (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of NoDup_map_inj_on`
   ...
   is `exact NoDup_map_inj_on.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_DeclarationBound.

Definition D__M_27_v1_restates_NoDup_map_inj_on_type := ltac:(let t := type of (@NoDup_map_inj_on) in exact t).
Lemma D__M_27_v1_restates_NoDup_map_inj_on : D__M_27_v1_restates_NoDup_map_inj_on_type.
Proof. exact (@NoDup_map_inj_on). Qed.

Print Assumptions D__M_27_v1_restates_NoDup_map_inj_on.
