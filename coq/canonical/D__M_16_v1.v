(* D/M.16.v1 -- mapped_not_wrapped -> wrapped -- restates profile_injective (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_DeclarationBound.v) -- parents: D *)
(* name: profile_injective (Theorem) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of profile_injective`
   ...
   is `exact profile_injective.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_DeclarationBound.

Definition D__M_16_v1_restates_profile_injective_type := ltac:(let t := type of (@profile_injective) in exact t).
Theorem D__M_16_v1_restates_profile_injective : D__M_16_v1_restates_profile_injective_type.
Proof. exact (@profile_injective). Qed.

Print Assumptions D__M_16_v1_restates_profile_injective.
