(* D/M.18.v1 -- mapped_not_wrapped -> wrapped -- restates two_pow_pos (information-discrete-math@147fc92671f35eb102405fec913eb361dc41f966:formal/IDM_DeclarationBound.v) -- parents: D *)
(* name: two_pow_pos (Lemma) *)
...
(* Toledo v1.7 cheap-win lane 1 (IDM wrap): restatement, not a bare
   alias. The type is obtained from Coq's own `type of two_pow_pos`
   ...
   is `exact two_pow_pos.` against the already-verified IDM mirror
   import below -- no new proof technique, no new claim. *)

From IDM.formal Require Import IDM_DeclarationBound.

Definition D__M_18_v1_restates_two_pow_pos_type := ltac:(let t := type of (@two_pow_pos) in exact t).
Lemma D__M_18_v1_restates_two_pow_pos : D__M_18_v1_restates_two_pow_pos_type.
Proof. exact (@two_pow_pos). Qed.

Print Assumptions D__M_18_v1_restates_two_pow_pos.
