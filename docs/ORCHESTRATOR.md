...
- Standards (BBL-197, "what world-class libraries use"): entries as JSON-LD (a `@context` mapping fields to schema.org/DataCite/PROV-O terms so the registry is also RDF); equations as LaTeX (source) + Presentation MathML (generated) + OpenMath/Content MathML where the statement is machine-parseable (generated, marked when not possible); graph as RDF/Turtle + GraphML; identifiers: stable URI per code (`https://morrocwi.github.io/toledo/<code>`) + release DOI; provenance via PROV-O terms (wasDerivedFrom = parents; wasAttributedTo = source record). Reference models: DLMF (LaTeX+MathML+metadata per formula), Lean mathlib/Coq docgen (source-linked pages), Wikidata (RDF ids), OpenMath content dictionaries.
- Statement layer = DLMF model (BBL-200): `latex/toledo.sty` defines semantic macros for programme symbols (\Readout, \Retain, \Live, \HumanReturn, \Weld …) each linked to its root code; `statement.latex` uses them; build generates Presentation MathML (latex2mathml, installed) and Content MathML/OpenMath via sympy where the statement parses (else `content_mathml: null` with reason); each entry page = DLMF-style formula record: statement · symbols (linked) · conditions · provenance · verification (Coq) · history/errata · permalink.
